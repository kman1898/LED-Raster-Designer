...
- **The tours' Go to restores shipped snapshots.** `src/static/data/tour_snapshots.json` holds every step's entry state for the three tours, generated by `python3 scripts/build_tour_snapshots.py`. Re-run it after changing a tour's steps or bumping the version; `tests/test_tour_snapshots.py` fails until you do.
