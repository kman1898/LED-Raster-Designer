#!/usr/bin/env python3
"""Export a saved project as a NovaStar .scr, from the command line.
...
def build_canvas_section(project, canvas, canvas_index, layers, warnings):
    ...
    for col in range(cols):
        for row in range(rows):
            ...
            if cab is None:
                panels.append({'col': col, 'row': row, 'port_num': 0, 'chain_order': 0,
                               'hidden': True, 'x': _int(xs[col]), 'y': y_of_cell,
                               'w': pw, 'h': ph})
            else:
                panels.append({'col': col, 'row': row,
                               'port_num': cab['port_num'] or 1,
                               'chain_order': cab['chain_order'],
                               'hidden': cab['hidden'],
                               'x': _int(cab['x']), 'y': y_of_cell,
                               'w': _int(cab['w']), 'h': _int(cab['h'])})

    # The record at binary (cols-1, rows-1) - app row 0, the section's top
    # right - is not a cabinet. Real files write a terminator there, and on a
    # section past the first the encoder writes an anchor. Either way a real
    # cabinet in that cell loses its routing, so it is called out.
    corner = grid.get((cols - 1, 0))
    if corner is not None and not corner['hidden']:
        # NOT REPRODUCED, and the one place this file knowingly differs from
        # the reference corpus. NovaStar's own EDC file makes room for the
        # terminator by sliding that row's data one column LEFT - its top row
        # runs cols 45-54 where the app's runs 46-55 - and extending the run by
        # a cabinet at the far end. The old exporter had a six-step heuristic
        # for it that only ever fired on anchor sections. Reproducing it needs
        # NovaLCT to confirm, so the cabinet is written where the app has it and
        # the cost is named here instead of being guessed at.
        warnings.warn(
            'terminator-%s' % canvas.get('id'),
            'canvas "%s": a live cabinet sits in the top-right grid cell, which the '
            'format reserves for the section terminator/anchor, so it loses its port. '
            'NovaStar\'s own files slide that row one column left to make room; this '
            'does not, so check that row in NovaLCT.' % canvas.get('name'))
