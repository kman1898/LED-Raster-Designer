...
# ── The port label is keyed by the port's own 1-based number ─────────────
#
# Mirror of the power-side ruling (2026-09-03, "should check data too for
# the same issue"): getPortLabelText(layer, n) for a 1-based port n must
# name port n everywhere - the HUD badge drawing under n, the bubble once n
# is drawn, the dock chip - and never its neighbour. On the data side the
# label rides the resolution keyed by the SCREEN's own port number
# (`number` = index + 1, the client's _indexAssignmentLabels reads exactly
# that key), and the port count sent is the highest number drawn, so a gap
# in the drawn numbers cannot slide the labels the way a gap in drawn
# circuits slid the plan-ordinal tails. Pinned across both box boundaries
# of an SX40 (ports 10/11 and 20/21, boxes A/B and B/C) and on a single
# card.

def test_a_port_is_labelled_by_its_own_number_across_the_sx40_boxes(client):
    """Twenty-two ports on a boxed SX40 with its four XDs named A-D: port
    10 is A-10 and port 11 is B-1, port 20 is B-10 and port 21 is C-1, and
    every entry's `number` is its 1-based position - the key the drawing
    reads the label back by."""
    ...
    assert (by_number[10], by_number[11]) == ('A-10', 'B-1'), got
    assert (by_number[20], by_number[21]) == ('B-10', 'C-1'), got
    assert got[9:11] + got[19:21] == ['A-10', 'B-1', 'B-10', 'C-1'], (
        'the list index and the number key must agree')
    # The return end of the same sockets, by the same key.
    by_return = {p['number']: p['returnLabel'] for p in scr['ports']}
    assert (by_return[10], by_return[11]) == ('A-10R', 'B-1R'), by_return


def test_a_port_is_labelled_by_its_own_number_on_a_single_card(client):
    """The single-card case the boxes are a special case of: a 20-port
    card named SR, ports 10 and 11 read SR-10 and SR-11 under numbers 10
    and 11."""
    state = add_processor(client, 'novastar-h9')
    pid = only(state)['id']
    state = set_card(client, pid, 0, 'novastar-card-h-20xrj45')
    ...
    assert (by_number[10], by_number[11]) == ('SR-10', 'SR-11')


def test_the_client_reads_the_label_back_under_the_same_number():
    """The lookup the canvas, the badge and the dock read is built keyed by
    port.number and read by the caller's 1-based port number - no ordinal,
    no offset, on either side."""
    src = js('app-port-assignment.js')
    ...
    assert "template.replace('#', portNum)" in fallback
    assert 'overrides[portNum]' in fallback
