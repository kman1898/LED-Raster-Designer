...
def test_positions_are_the_groups_and_the_rows_read_in_the_sheets_vocabulary(page):
    ...
    assert totals[('H9', 'EA')] == 1
    assert len(out['totals']) == 18
...
def test_the_per_screen_readings_the_packet_will_print(page):
    ...
    assert (box['distro'], box['number'], box['homeRun'], box['type'], box['connector'], box['shared']) == (
        'SR', 1, '125', 'Multi 208', 'Tru-1', False)
...
def test_a_snake_of_two_with_two_extensions_is_two_barrels(page):
    ...
    assert out['positions'] == [
        ['SR Beach', [
            ...
            {'type': 'H9', 'length': 'EA', 'qty': 1, 'label': '', 'notes': '', 'side': 'data'}]],
        ['CENTER', [
...
def test_smoke_experts_only(page):
    ...
    and a loose 100' on A-5, SR - Return's port; box SR B on Card 3 is
    the backup end, walked like the primary: snake SR B 100', extensions
    on B-1 and B-3 (10') and B-4 (75'), a loose 50' on B-5 - one barrel
