...
def test_toggle_panel_hidden(client_with_layer):
    ...
    v0.10.8.1: the route rebuilds the layer's geometry (hidden state anchors
    half-tiles), so it returns the rebuilt LAYER rather than the panel - the
    panel object is regenerated by the rebuild.
    """
    project = client_with_layer.get('/api/project').get_json()
    layer_id = project['layers'][0]['id']
    panel_id = project['layers'][0]['panels'][0]['id']
    ...
    def _hidden_of(resp):
        ...
        return next(p for p in layer['panels'] if p['id'] == panel_id)['hidden']

    resp = client_with_layer.post(f'/api/layer/{layer_id}/panel/{panel_id}/toggle_hidden')
    assert resp.status_code == 200
    assert _hidden_of(resp) is True

    resp = client_with_layer.post(f'/api/layer/{layer_id}/panel/{panel_id}/toggle_hidden')
    assert resp.status_code == 200
    assert _hidden_of(resp) is False
