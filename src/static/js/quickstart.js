...
(function () {
    ...
    var WHATSNEW_STEPS = [
        ...
        { target: '#hardware-dock-body .hw-dock-gear', place: 'top', title: 'Configuration behind the &#9881;',
          body: 'Each header&rsquo;s <b>&#9881;</b> opens its settings popover: card slots, breakout boxes, label templates, remove. <b>Redundancy</b> is one bar behind the <b>processor&rsquo;s</b> &#9881; only: <b>Off &middot; Whole unit &middot; Per card &middot; Per port</b>.' },
        ...
        { target: 'canvas', place: 'top', title: 'Right-click to clear',
          body: 'Right-click a drawn run or a dock chip for <b>Clear port</b> / <b>Clear multi</b> &mdash; the port comes off its card, undoably, and stays unattached until you place it again. The menu only ever offers what applies.' },
        ...
        { target: '[data-mode="power"]', place: 'bottom', title: 'Power works the same way', before: function () { switchView('power'); },
          body: 'Distros sit in the tray with their multis. Drag a <b>multi</b> onto a circuit to land it &mdash; drop on a later circuit and the multi <b>splits there</b>; the drop is the boundary. Each box wears a <b>type chip</b> (Soca 208, Soca 120, L21-30): click the spare box&rsquo;s to pick, and it lands as that plug. Drag one chip to put a single circuit on one circuit of the box.' },
    ...
    var ADVANCED_STEPS = [
        ...
        { target: '#hardware-dock-body .hw-dock-gear', place: 'top', title: 'Redundancy, one bar',
          body: 'Behind the processor&rsquo;s &#9881; one bar sets it: <b>Off &middot; Whole unit &middot; Per card &middot; Per port</b> (a standalone unit reads <b>Off &middot; Backed up &middot; Per port</b>). <b>Whole unit</b> &mdash; pick the unit that mirrors this one, card for card. <b>Per card</b> &mdash; a partner per slot. <b>Per port</b> &mdash; <b>Sequential</b> (1 backed by 2, 3 by 4), <b>Halves</b> (the back half carries the front half&rsquo;s returns) or <b>Manual</b> (each port picks its backup on its chip). The gold pill on a header reads the state; click it to open the bar.' },
        { target: 'canvas', place: 'top', title: 'Nothing lands by itself',
          body: 'A port is on a card only because you put it there: a <b>drop pins</b> it to the socket you chose, and it stays until you clear it. Right-click &rsaquo; <b>Clear</b> or a drag back to the tray releases it, and the flag counts what is still unattached. Nothing is ever moved or unpinned silently.' },
        ...
        { target: '#power-distro-add', place: 'bottom', title: 'Distros &amp; multis',
          body: '<b>+ Add distro</b> puts a power source in the tray. Drag a <b>multi</b> onto a circuit to land it; drop on a later circuit and it <b>splits there</b>. Every box wears its connector as a <b>type chip</b> &mdash; Soca 208, Soca 120, L21-30 &mdash; click the spare box&rsquo;s chip to pick, and the box lands as that plug. Drag a whole distro onto a screen to feed all its unassigned multis at once.' },
        ...
        { target: '#hardware-dock-body .hw-dock-tile', place: 'top', title: 'Circuit chips',
          body: 'Each circuit of the box is a chip and every chip is a meter of its circuit&rsquo;s load. Click an occupied chip to rename that circuit; a clash (two claims on one circuit) is said out loud on the chip and in the issue rows.' },
