...
(function () {
    ...
    var WHATSNEW_STEPS = [
        ...
        { target: '#hardware-dock-body', place: 'top', title: 'Name things on their headers',
          body: 'Every header holds its own name box. Name a card <b>SR</b> and its ports read SR-1, SR-2; name a distro <b>SL</b> and its multis are SL1, SL2 &mdash; and every label on the wall follows. Everything electrical sits behind each header&rsquo;s <b>&#9881;</b>, with <b>Remove</b> in a footer that never scrolls out of reach.' },
        { target: 'canvas', place: 'top', title: 'Nothing lands by itself',
          body: 'Auto-numbering is retired. A port is on a card only because you put it there: drag a <b>port</b> onto a screen&rsquo;s run, or a whole <b>card</b> or <b>box</b> onto a screen to fill its ports in order. Right-click &rsaquo; <b>Clear</b>, or a drag back to the tray, releases it. Nothing is ever moved or unpinned silently.' },
        { target: '#hardware-dock-body .hw-dock-tile', place: 'top', title: 'A port prints its socket',
          body: 'An attached port is labelled by the socket it sits on &mdash; <b>6, 7, 8, 9</b> on an unnamed card, <b>H9-6</b> on a card named H9 &mdash; the same numbers the tray shows. A port on no hardware keeps the screen&rsquo;s own P1, P2 numbering. Click a chip to rename that one port.' },
        { target: '#hw-dock-flag', place: 'bottom', title: 'The attachment flag',
          body: '<b>Red</b> with a screen count while any screen still has ports or circuits off the hardware; <b>green</b> when everything is held. Click it to list the screens, and click a row to fly the canvas there.' },
        ...
        { target: '#hardware-dock-body .hw-dock-proc-name', place: 'top', title: 'One zone beneath, and a gold pill',
          body: 'Exactly one thing sits under the bar: a <b>mirrored by</b> pick for the whole unit, one partner per slot for cards, or <b>Sequential &middot; Halves &middot; Manual</b> chips per slot for ports. Once set, every tray header wears a gold pill that reads the shape &mdash; <b>R &rarr; H9 BACKUP</b>, <b>R seq</b>, <b>backs up SR</b> &mdash; and clicking it opens the bar. A card&rsquo;s own &#9881; only states its shape.' },
        { target: '#hardware-dock-body .hw-dock-grid', place: 'top', title: 'Snake the ports',
          body: 'Hold <b>Alt</b> and sweep across the port chips of one card or box, then right-click &rsaquo; <b>Snake these N</b>. The snake reads as a blue bracket under its ports &mdash; <b>SNAKE A &middot; 6-way &middot; 100&rsquo;</b> &mdash; one name, one home run, however many ports ride it. A snake belongs to the socket, so clearing a port off a wall keeps it.' },
        { target: '#hardware-dock-body .hw-dock-cablebtn-data', place: 'top', title: 'Every port carries its home run',
          body: 'The <b>&#8801;</b> on a card or box flips its chips into a cable sheet: tick ports and press <b>Snake</b> to make one here too, name it, type its home run, and give each loose port its own length and connector &mdash; <b>Tab</b> walks the column. A loose port wears its length in its chip corner, and <b>Show Cable Tags</b> in the Data panel prints <b>SNAKE A</b> or <b>50&rsquo; CAT</b> beside the port on the wall and in the export.' },
        ...
        { target: '#hardware-dock-body .hw-dock-multi', place: 'top', title: 'A multi takes what its breakout has free',
          body: 'Drag a multi over a wall and the span starts at the <b>first circuit</b> of that six and grows to the one under your cursor &mdash; one, then two, then six, in order &mdash; capped at what the breakout has free: six on an empty soca, three on an L21-30, less whatever is already on it. Short, it says <b>took 5 of 6</b>. Clear a multi and the wall welds back to its natural six-per-breakout grid; one undo restores it all.' },
        { target: '#hardware-dock-body .hw-dock-typechip', place: 'top', title: 'A breakout wears its type',
          body: 'Every breakout carries a chip &mdash; <b>Soca 208</b>, <b>Soca 120</b>, <b>L21-30</b>. On the spare breakout the chip is the picker: click it to cycle, drag that breakout onto a screen and it lands as that plug. An occupied breakout reads the type its circuits imply, and a spare breakout follows the distro&rsquo;s other breakouts, so an Edison distro&rsquo;s next breakout is Edison without a click.' },
        { target: '#hardware-dock-body .hw-dock-outputs', place: 'top', title: 'OUTPUTS: the plugs you drag',
          body: 'Tick the connector types a distro offers behind its &#9881; and each becomes a plug on this row. Drag one onto a screen: the circuits the drop will feed light up with a pending bracket and a pill &mdash; <b>SL 3 &rarr; circuits 7-12 &middot; 81 A</b>. A screen set to a different breakout refuses with the reason, and the pill warns when the breakout would push the distro&rsquo;s legs past rating.' },
        { target: '#hardware-dock-body .hw-dock-tile', place: 'top', title: 'Three grains, three clears',
          body: 'Right-click a circuit chip (or drag it back onto the tray) and <b>Clear circuit SR1-6</b> takes that one circuit off the multi; the other five stay put and keep their labels. The breakout header clears the whole multi, and the distro clears everything. Chip = circuit, header = multi, distro = the lot.' },
        { target: '#hardware-dock-body .hw-dock-multi', place: 'top', title: 'Every circuit carries its cable',
          body: 'The multi&rsquo;s name and its one home-run length live on its header. On a breakout with circuits on it, a raised <b>&#8801;</b> flips the chips into a cable sheet &mdash; a row per circuit with its screen, <b>feet</b> and <b>connector</b>, <b>Tab</b> walking the column, quick fills for the whole breakout and a count by length and connector at the foot. The connector follows the breakout, so mostly you type a number; a chip wears its cable in its corner when the sheet is closed.' },
        { target: '#show-power-cable-tags', place: 'right', title: 'Show Cable Tags',
          body: 'Per screen, off by default: draws each circuit&rsquo;s cable as a gold tag &mdash; <b>10&rsquo; True1</b> &mdash; beside its label on the wall and in the export, for the per-screen docs. The Data panel has the same switch for snakes and home runs.' },
        { target: '#show-power-nfer-tags', place: 'right', title: 'Gang runs, and tag them or not',
          body: 'Hold <b>Alt</b> and drag across cabinets on the wall, then right-click &rsaquo; <b>2fer</b> or <b>3fer</b> to gang those runs onto one circuit. Untick <b>Show 2fer / 3fer Tags</b> to keep the bracket and drop the text &mdash; a gang past the screen&rsquo;s amps still prints <b>OVER</b>.' },
        { target: '#power-custom-toggle', place: 'right', title: 'Drawing stops at capacity',
          body: 'In custom mode a circuit takes only as many cabinets as its settings allow &mdash; a click past full is refused and says so. Select a block and apply a flow pattern and it deals the block out at capacity: 14 wide by 6 tall on a 14-panel circuit becomes circuits 1 through 6, every circuit starting from the same side. Same on the Data side with ports.' },
        { target: '#btn-fit', place: 'bottom', title: 'The readout moved into the strip',
    ...
    var ADVANCED_STEPS = [
        ...
        { target: '[data-mode="power"]', place: 'bottom', title: 'Power view', before: function () { switchView('power'); },
          body: 'Plan electrical distribution. The left panel is this screen&rsquo;s electrical math and styling; the tray holds the <b>distros</b>, and dragging multis and chips is how circuits land on real breakouts.' },
        ...
        { target: '#power-breakout-type', place: 'right', title: 'Breakouts',
          body: 'How the multi terminates: <b>True1</b> / <b>powerCON</b> feed panels directly, <b>Edison</b> is the 110V option, <b>L6-20</b> adds L6-20-to-panel cables per circuit, and <b>L21-30 (3 &times; 208V)</b> is a three-circuit breakout at 30&nbsp;A per leg. Drives the gear checklist.' },
        ...
        { target: '#power-distro-add', place: 'bottom', title: 'Distros &amp; multis',
          body: '<b>+ Add distro</b> puts a power source in the tray. Drag a <b>multi</b> over a wall and it takes from the <b>first circuit</b> of that six up to the one under your cursor, capped at what the breakout has free (<b>took 5 of 6</b> when short); clearing a multi welds the wall back to its six-per-breakout grid. Every breakout wears its connector as a <b>type chip</b> &mdash; Soca 208, Soca 120, L21-30 &mdash; click the spare breakout&rsquo;s chip to pick, and the breakout lands as that plug; the distro&rsquo;s <b>OUTPUTS</b> row holds the same plugs with a live drop preview. Drag a whole distro onto a screen to feed all its unassigned multis at once.' },
        { target: '#hardware-dock-body [data-hwdock^="slot-"]', place: 'top', title: 'One soca, two screens',
          body: 'Drop a multi onto an <b>occupied</b> breakout and they become one physical soca: same distro, same number. The incumbents keep their circuits, the joiner takes the free ones, and every label derives from the one breakout name.' },
        ...
        { target: '#hardware-dock-body .hw-dock-tile', place: 'top', title: 'Circuit chips',
          body: 'Each circuit of the breakout is a chip and every chip is a meter of its circuit&rsquo;s load. Click an occupied chip to rename that circuit; right-click it for <b>Clear circuit</b>, which takes that one circuit off the breakout and leaves the other five (the header clears the multi, the distro clears everything). A clash (two claims on one circuit) is said out loud on the chip and in the issue rows.' },
        { target: '#hardware-dock-body .hw-dock-multi', place: 'top', title: 'Cable sheets',
          body: 'The multi&rsquo;s name and home-run length live on its header. On a breakout with circuits on it the raised <b>&#8801;</b> flips the chips into a cable sheet &mdash; feet and connector per circuit, <b>Tab</b> down the column, quick fills and a count at the foot; a chip wears its cable in its corner when the sheet is closed. <b>Show Cable Tags</b> (per screen, off by default, in both the Power and Data panels) prints them beside the labels on the wall and in the export.' },
