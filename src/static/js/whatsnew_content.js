...
window.WHATS_NEW_CONTENT = {
    '0.12': {
        ...
        items: [
            ...
            { h: 'Ports snake',
              d: 'Hold Alt and sweep the port chips of one card or box, then right-click Snake these N - or tick them in the card’s cable sheet and press Snake. A snake reads as a blue bracket (SNAKE A · 6 channel · 100’) under its ports, a loose port carries its own length, a snaked port can carry an ext length for a shorter extension off the fan-out, and a backup port’s run is typed on the backup card’s or box’s own sheet and counted on the papers. The papers pull gear where its distro or breakout box sits - a Beach picked on the gear, on each screen in Screen Info, or made on the spot; a project keeps its beaches as a list, in the order the pull sheet and binder run - and list the CVTs and distros themselves. The Data panel’s Show Cable Tags prints them on the wall. The binder is a drawing set now: numbered sheets by subject (1.x overview, 2.x the screens in beach order and, within a beach, the Screen order you pick in the export dialog - alphabetical unless you say otherwise - each one’s power sheet and its power wiring sheet, then its data sheet and its data wiring sheet, 3.x the pull sheets with positions side by side, 4.x hardware) that grow to fill their sheet with a title block down the right edge - your logo, a revision log written on export, venue, dates, designer, project manager, drafter - each map a numbered view with its tables beside it, on Tabloid 17 x 11 by default or any sheet size, the text real text in the PDF. Behind each map is its wiring sheet, on a page of its own - Power Wiring after the power sheet, Data Wiring after the data sheet, one Wiring tick for both, each following the Maps choice - and it draws the wall over what its circuits or ports land on: a run leaves the wall’s own label disc - no second tag over the panels - travels out at its own row and drops into its socket on the unit standing across the foot of the sheet, the units ordered by where their own runs stand and named inside themselves, and a run whose socket sits under its own column steps out past the wall’s edge and comes back rather than dropping through the discs below it. Every port is wired to its socket on its box or card, backups included, every circuit to its slot on the multi’s breakout; white casing under every line keeps it readable over the panels, and the printer palette tells the devices apart by a dash pattern per device instead of a hue. The tray itself is a fixed grid of columns: a unit keeps its place and its width whatever it is doing, so opening a cable sheet or folding a section only changes height and never shuffles the row - and the ⋮⋮ on a processor’s or a distro’s line drags along the tray to reorder them, with the papers following the order you set. The binder’s Frame switch turns the border and the title block column off altogether, so the drawing takes the whole sheet inside a small margin and the numbered view bubble with its heading at the foot is what names it.' }
        ]
    },
    '0.11': {
        title: 'Screen groups',
